...
def execute(ARGS):
	...
	if hasSVGO is None:
		...
		if filepath:
			helpers.run_command('svgo {FILE} -o {FILE}.optimized'.format(FILE = filepath))
		else:
			...
			if batch:
				for svgFile in orderedFileList:
					...
					helpers.run_command("svgo '{DIR}/{FILE}' -o '{DESTINATION}/{FILE_CORE}.clean.svg'".format(DIR = dir.replace('\n', ''), DESTINATION = destination.replace('\n', ''), FILE = svgFile, FILE_CORE = svgFileCore))
			else:
